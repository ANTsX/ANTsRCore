  ...
  if (!is.character(fixed)) {
    ...
    if (is.antsImage(fixed) & is.antsImage(moving)) {
      ...
      if (ttexists) {
        ...
        if( is.list( mask ) ) 
          {
          fixedMaskBinary <- thresholdImage( mask[[1]], 0, 0, 0, 1 )
          fMaskStr <- antsrGetPointerName( fixedMaskBinary )

          movingMaskBinary <- thresholdImage( mask[[2]], 0, 0, 0, 1 )
          mMaskStr <- antsrGetPointerName( movingMaskBinary )          
          } else {

          if( !is.null( mask ) ) 
            {
            fixedMaskBinary <- thresholdImage( mask, 0, 0, 0, 1 )
            fMaskStr <- antsrGetPointerName( fixedMaskBinary )
            }
          if( !is.null( movingMask ) ) 
            {
            movingMaskBinary <- thresholdImage( movingMask, 0, 0, 0, 1 )
            mMaskStr <- antsrGetPointerName( movingMaskBinary )
            }
          }
